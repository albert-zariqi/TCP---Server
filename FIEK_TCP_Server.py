...
def clientThread(connenctionSocket,addr):
    
    
        while 1:
            try:  
                #Pranimi i kerkeses nga klienti
                kerkesa = connenctionSocket.recv(128)
                #perpunimi i kerkeses se klientit, dhe dergimi i pergjigjes tek klienti permes funksionit send 
                kerkesa01 = str(kerkesa)
                kerkesa01 = kerkesa01[2:]
                kerkesa01 = kerkesa01[:-1]
                kerkesa01 = kerkesa01.replace("\\t"," ")
                kerkesa01 = kerkesa01.replace("   "," ")
                kerkesa01 = kerkesa01.replace("  "," ")
                kerkesa01 = kerkesa01.strip()
                
                kerkesa01 = kerkesa01.split(" ")
                kerkesa02 = " ".join(kerkesa01)
                if(kerkesa01[0] == "IPADDR"):
                    pergjigja = IPADDR(addr)
                    pergjigja01 = "IP Adresa e klientit eshte " + str(pergjigja)
                    connenctionSocket.send(str.encode(pergjigja01))
                elif(kerkesa01[0] == "PORTNR"):
                    pergjigja = PORTNR(addr)
                    pergjigja01 = "Klienti eshte duke perdorur portin " + str(pergjigja)
                    connenctionSocket.send(str.encode(pergjigja01))
                elif(kerkesa01[0] == "LOJA"):
                    pergjigja = LOJA()
                    connenctionSocket.send(str.encode(pergjigja))
                elif(kerkesa01[0] == "PALINDROMET"):
                    print(kerkesa01)
                    pergjigja = PALINDROMET(kerkesa01)
                    connenctionSocket.send(str.encode(pergjigja))
                elif(kerkesa01[0] == "EKKUADRATIK"):
                    pergjigja = EKKUADRATIK(kerkesa01)
                    connenctionSocket.send(str.encode(pergjigja))
                elif(kerkesa01[0] == "FIBONACCI"):
                    pergjigja = FIBONACCI(kerkesa01)
                    pergjigja = str(pergjigja)
                    connenctionSocket.send(str.encode(pergjigja))
                
                elif(kerkesa01[0] == "ZANORE"):
                    pergjigja = ZANORE(kerkesa01)
                    connenctionSocket.send(str.encode(pergjigja))
                elif(kerkesa01[0] == "PRINTO"):
                    pergjigja = PRINTO(kerkesa01)
                    connenctionSocket.send(str.encode(pergjigja))
                elif(kerkesa01[0] == "HOST"):
                    pergjigja = HOST()
                    pergjigja01 = "Emri i hostit eshte " + pergjigja
                    connenctionSocket.send(str.encode(pergjigja01))
                
                elif(kerkesa01[0] == "KONVERTO"):
                    pergjigja = KONVERTO(kerkesa01)
                    pergjigja = str(pergjigja)
                    connenctionSocket.send(str.encode(pergjigja))

                elif(kerkesa01[0] == "TIME"):
                    pergjigja = TIME()
                    connenctionSocket.send(str.encode("Koha aktuale eshte " + pergjigja))
                elif(kerkesa01[0] == "ENKRIPTIMI"):
            
                    pergjigja = ENKRIPTIMI(kerkesa02)
                    connenctionSocket.send(str.encode(str(pergjigja)))
                else:
                    connenctionSocket.send(str.encode("BAD REQUEST!"))
...
#Numri maksimal i lidhjeve qe mund te behen ne socketin perkates
serverSocket.listen(5)
print("Serveri eshte i gatshem te pranoj kerkesa")


while True:
    connenctionSocket, addr = serverSocket.accept()
    print("Klienti me ip adrese %s dhe me port %s u lidh ne server "%(addr[0],addr[1]))
    #krijimi i thread-ve
    start_new_thread(clientThread, (connenctionSocket,addr))
serverSocket.close()